...
import { useTranslation, useKeyValueStore, useDateTimeFormat, ContextMenuButton, Translate, usePerspective, useDataset, useScheduledDraftsEnabled, useActiveReleases, useWorkspace, isCardinalityOneRelease, sortReleases, getReleaseIdFromReleaseDocumentId, useClient } from "sanity";
...
function VisionGui(props) {
  const {
    ...
    data: releases = []
  } = useActiveReleases(), isDraftModelEnabled = useWorkspace().document.drafts.enabled, editorQueryRef = useRef(null), editorParamsRef = useRef(null), visionRootRef = useRef(null), customApiVersionElementRef = useRef(null), querySubscriptionRef = useRef(void 0), listenSubscriptionRef = useRef(void 0), [localStorage2] = useState(() => getLocalStorage(projectId || "default")), storedDataset = localStorage2.get("dataset", defaultDataset), storedApiVersion = localStorage2.get("apiVersion", prefixApiVersion(`${config.defaultApiVersion}`)), storedQuery = localStorage2.get("query", ""), storedParams = localStorage2.get("params", `{
  
}`), storedPerspective = localStorage2.get("perspective", void 0), [dataset, setDataset] = useState(() => datasets.includes(storedDataset) ? storedDataset : datasets.includes(defaultDataset) ? defaultDataset : datasets[0]), [apiVersion, setApiVersion] = useState(() => API_VERSIONS.includes(storedApiVersion) ? storedApiVersion : DEFAULT_API_VERSION), [customApiVersion, setCustomApiVersion] = useState(() => API_VERSIONS.includes(storedApiVersion) ? !1 : storedApiVersion), [perspective, setPerspectiveState] = useState(storedPerspective || "raw"), isValidApiVersion = customApiVersion ? validateApiVersion(customApiVersion) : !0, [url, setUrl] = useState(void 0), [query, setQuery] = useState(() => typeof storedQuery == "string" ? storedQuery : ""), [rawParams, setRawParams] = useState(storedParams), params = useMemo(() => parseParams(rawParams, t), [rawParams, t]), [queryResult, setQueryResult] = useState(void 0), [listenMutations, setListenMutations] = useState([]), [error, setError] = useState(void 0), [queryTime, setQueryTime] = useState(void 0), [e2eTime, setE2eTime] = useState(void 0), [queryInProgress, setQueryInProgress] = useState(!1), [listenInProgress, setListenInProgress] = useState(!1), [isQueryRecallCollapsed, setIsQueryRecallCollapsed] = useState(!1), {
    paneSizeOptions,
    isNarrowBreakpoint
  } = usePaneSize({
    visionRootRef
  }), scheduledDraftsStack = useMemo(() => {
    if (!isScheduledDraftsEnabled) return;
    const scheduledDraftReleases = releases.filter((release) => isCardinalityOneRelease(release) && release.state === "scheduled"), releaseIds = sortReleases(scheduledDraftReleases).map((release_0) => getReleaseIdFromReleaseDocumentId(release_0._id)), defaultPerspective = isDraftModelEnabled ? ["drafts"] : ["published"];
    return [...releaseIds, ...defaultPerspective];
  }, [releases, isDraftModelEnabled, isScheduledDraftsEnabled]), _client = useClient({
    ...
    dataset,
    allowReconfigure: !0
  }), [perspectiveStack, scheduledDraftsStack, perspective, customApiVersion, apiVersion, dataset, _client, isValidApiVersion]), cancelQuerySubscription = useCallback(() => {
  ...
  }, [queryInProgress, query, dataset, params.parsed, t, perspective, perspectiveStack, scheduledDraftsStack, customApiVersion, isValidApiVersion, apiVersion, localStorage2, cancelListenerSubscription, client, cancelQuerySubscription]), setPerspective = useCallback((newPerspective) => {
