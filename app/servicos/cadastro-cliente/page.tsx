...
import { UserPlus, ExternalLink, ArrowRight } from 'lucide-react'
import { Card, CardContent } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import Link from "next/link"
import { useI18n } from "@/lib/i18n/context"
import { useCadastroClienteData } from "@/hooks/useSanityData"
import { getTranslatedField } from "@/lib/sanity-i18n"
import { PortableText } from "@/components/portable-text"

export default function CadastroClientePage() {
  const { t, language } = useI18n()
  const { data: pageData } = useCadastroClienteData()

  const title = getTranslatedField(pageData?.title, language, t("clientRegistration"))
  const description = getTranslatedField(pageData?.description, language, t("clientRegistrationDescription") || "Cadastre-se para acessar nossos serviços digitais")
  const linkSistema = pageData?.linkSistema || "https://api.portoitapoa.com/hub-frontend/public-cadastro-clientes"
  ...
  return (
    <div className="min-h-screen mt-44 py-16 px-8 bg-white text-gray-900">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Embed direto do formulário (integração) */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <Card className="border border-gray-200 shadow-xl rounded-2xl overflow-hidden">
            <CardContent className="p-0">
              <div className="relative w-full overflow-hidden" style={{ height: "2200px" }}>
                <iframe
                  src={linkSistema}
                  title="Cadastro de Clientes - Porto Itapoá"
