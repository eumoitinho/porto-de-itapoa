...
import { useIntegracaoServicosData } from "@/hooks/useSanityData"
import { getTranslatedField } from "@/lib/sanity-i18n"
import { PortableText } from "@/components/portable-text"

export default function IntegracaoServicosPage() {
  const { t, language } = useI18n()
  const { data: pageData } = useIntegracaoServicosData()

  const title = getTranslatedField(pageData?.title, language, t("serviceIntegration") || "Integração de Serviços")
  const description = getTranslatedField(pageData?.description, language, t("serviceIntegrationDescription") || "Integre-se como terceiro para acessar nossos serviços")
  const linkSistema = pageData?.linkSistema || "https://forms.office.com/Pages/ResponsePage.aspx?id=QwqdHgbzNk2eLbcz8M_rfv0DXNnoEF1DjZa7wG4kOlNUMzdJMlNIOTIwTjY0NVM2RDJGMDJEVVRLTCQlQCN0PWcu"
        ...
        >
          <Card className="backdrop-blur-md bg-gradient-to-r from-green-600 to-emerald-700 border-0 shadow-xl rounded-2xl">
            <CardContent className="p-8 text-white">
              <Link2 className="h-16 w-16 mx-auto mb-6" />
              <h2 className="text-3xl font-semibold mb-4">
                {t("accessIntegration") || "Acesse a Integração"}
              </h2>
              <p className="text-green-100 mb-6 max-w-3xl mx-auto">
                {t("serviceIntegrationDescription") || "Integre-se como terceiro para acessar nossos serviços"}
              </p>
